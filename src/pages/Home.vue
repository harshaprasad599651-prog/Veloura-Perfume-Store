...
<template>
  <div
    ...
    :class="[
      ...
      isDark
        ? 'bg-linear-to-br from-[#050008] via-[#16001f] to-[#030005]'
        : 'bg-linear-to-br from-pink-50 via-white to-purple-100',
    ]"
  >
    <AppHeader />
    ...
    <section class="relative h-[85vh] overflow-hidden">
      ...
      <div class="absolute inset-0 bg-linear-to-r from-black/75 via-black/40 to-pink-500/20"></div>

      <div class="absolute inset-0 z-10 flex flex-col items-center justify-center px-6 text-center">
        <p class="mb-6 font-serif text-[24px] font-semibold tracking-[0.35em] text-pink-100 drop-shadow-[0_2px_10px_rgba(255,192,203,0.6)]">
          VELOURA PERFUME STORE
        </p>

        <h1 class="max-w-6xl text-5xl font-extrabold uppercase leading-tight text-white drop-shadow-[0_6px_20px_rgba(0,0,0,0.8)] md:text-8xl">
          {{ heroSlides[currentSlide].title }}
        </h1>

        <p class="mt-8 max-w-3xl text-xl text-pink-50 drop-shadow-[0_2px_8px_rgba(0,0,0,0.6)] md:text-2xl">
      ...
      <div class="absolute bottom-8 left-1/2 z-20 flex -translate-x-1/2 gap-3 rounded-full bg-white/90 px-5 py-3 shadow-xl">
        <button
          v-for="(_, index) in heroSlides"
          :key="index"
          type="button"
          class="h-3 w-3 rounded-full transition duration-300"
          :class="index === currentSlide ? 'scale-125 bg-cyan-500' : 'bg-gray-500'"
    ...
    <main class="mx-auto max-w-7xl px-6 py-14">
      ...
      <section class="mt-14 grid grid-cols-1 gap-8 md:grid-cols-3">
        ...
        >
          <h2 class="font-serif text-3xl font-bold text-white!">Women</h2>
          <p class="mt-2 text-pink-100!">
            Elegant floral and sweet luxury scents.
          </p>
        </RouterLink>

        <div class="rounded-3xl bg-linear-to-br from-slate-900 to-purple-900 p-8 text-white shadow-xl transition duration-300 hover:-translate-y-2 hover:shadow-[0_0_25px_rgba(168,85,247,0.8)]">
          <h2 class="font-serif text-3xl font-bold text-white!">Men</h2>
          <p class="mt-2 text-gray-300!">
            Bold, fresh, and confident fragrances.
          </p>
        </div>

        <div class="rounded-3xl bg-linear-to-br from-cyan-500 to-pink-500 p-8 text-white shadow-xl transition duration-300 hover:-translate-y-2 hover:shadow-[0_0_25px_rgba(6,182,212,0.8)]">
          <h2 class="font-serif text-3xl font-bold text-white!">Unisex</h2>
